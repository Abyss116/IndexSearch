...
$(BIN): src/main.rs src/frontend.rs src/bin/indexsearch.rs src/bin/is.rs src/bin/isgrep.rs src/bin/istool.rs Cargo.toml
	cargo build --release
	cp target/release/indexsearch $(BIN)
	cp target/release/is is
	cp target/release/isgrep isgrep
	...
	@if [ "$$(uname -s)" = "Darwin" ]; then codesign --force --sign - isgrep >/dev/null 2>&1 || true; fi
...
clean:
	rm -f $(BIN) is isgrep $(DAEMON) $(TOOL)
	cargo clean
