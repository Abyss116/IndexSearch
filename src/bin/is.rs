...
fn run(args: &[String]) -> Result<i32, String> {
    ...
    if let Some(record) = read_valid_record(&root) {
        if let Ok(output) = request_daemon(&record, &daemon_args) {
...
fn request_daemon(record: &DaemonRecord, args: &[String]) -> io::Result<SearchOutput> {
    ...
    let stdout = read_frame(&mut stream)?;
    let stderr = read_frame(&mut stream)?;
    Ok(SearchOutput {
        code,
        stdout,
        stderr,
...
fn write_output(output: SearchOutput) -> Result<i32, String> {
    io::stdout()
        .lock()
...
fn backend_path() -> Result<PathBuf, String> {
    ...
    let sibling = dir.join(executable_name("indexsearch"));
    if sibling.is_file() {
        return Ok(sibling);
    }
    if exe
        ...
        .is_some_and(|stem| stem == "indexsearch")
    {
        return Ok(exe);
    }
    Err(format!(
